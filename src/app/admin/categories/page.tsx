                          ...
                          <tr key={cat.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                  ...
                                  <div className="flex flex-col gap-1">
                                                  ...
                                                  <select value={editParentId} onChange={(e) => setEditParentId(e.target.value)}
                                                                    ...
                                                                    <option value="">Sin categoría padre</option>option>
                                  ...
                                  <span className="line-clamp-1">{cat.description || '-'}</span>span>
                                                )}
                                    </td>td>
                                    <td className="px-4 py-3 text-right">
                                                <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">{cat.product_count}</span>span>
                                    </td>td>
                                    <td className="px-4 py-3 text-right">
                                      {editingId === cat.id ? (
                                  <div className="flex items-center justify-end gap-1">
                                                  ...
                                                  <button onClick={() => setEditingId(null)} className="px-3 py-1 text-xs border border-gray-300 rounded hover:bg-gray-50">Cancelar</button>button>
                                  </div>div>
                                ) : (
                                  <div className="flex items-center justify-end gap-1">
    ...
    return (
          <div className="space-y-4">
                <div className="flex items-center justify-between">
                        <div>
                                  <h2 className="text-2xl font-bold text-gray-900">Categorías</h2>h2>
                                  <p className="text-sm text-gray-500 mt-1">{totalCount} categorías registradas</p>p>
                        </div>div>
            ...
            {showForm && (
                    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
                              <h3 className="font-semibold text-gray-800">Nueva categoría</h3>h3>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                          ...
                                          <select value={newParentId} onChange={e => setNewParentId(e.target.value)}
                                                          className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                        <option value="">Sin categoría padre (categoría raíz)</option>option>
                                            {allFlat.map(f => (
                                                                            <option key={f.id} value={f.id}>{'\u00a0'.repeat(f.depth * 2)}{f.name}</option>option>
                                                                          ))}
                                          </select>select>
                        ...
                        <table className="w-full text-sm">
                                  <thead>
                                              <tr className="bg-gray-50 border-b border-gray-200">
                                                            <th className="px-4 py-3 text-left font-medium text-gray-500">Nombre</th>th>
                                                            <th className="px-4 py-3 text-left font-medium text-gray-500 hidden md:table-cell">Slug</th>th>
                                                            <th className="px-4 py-3 text-left font-medium text-gray-500 hidden lg:table-cell">Descripción / Padre</th>th>
                                                            <th className="px-4 py-3 text-right font-medium text-gray-500">Productos</th>th>
                                                            <th className="px-4 py-3 text-right font-medium text-gray-500 w-24">Acciones</th>th>
                                              </tr>tr>
                                  </thead>thead>
                                  <tbody>
                                    {loading ? (
                          Array.from({ length: 6 }).map((_, i) => (
                                            <tr key={i} className="border-b border-gray-100 animate-pulse">
                                                              <td className="px-4 py-3"><div className="h-4 bg-gray-200 rounded w-32" /></td>td>
                                                              <td className="px-4 py-3 hidden md:table-cell"><div className="h-4 bg-gray-200 rounded w-24" /></td>td>
                                                              <td className="px-4 py-3 hidden lg:table-cell"><div className="h-4 bg-gray-200 rounded w-48" /></td>td>
                                                              <td className="px-4 py-3"><div className="h-4 bg-gray-200 rounded w-10 ml-auto" /></td>td>
                                                              <td className="px-4 py-3"><div className="h-4 bg-gray-200 rounded w-16 ml-auto" /></td>td>
                                            </tr>tr>
                                          ))
                        ) : (
